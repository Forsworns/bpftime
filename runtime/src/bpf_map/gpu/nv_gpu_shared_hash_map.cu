...
#define MINIMUM_CUDA_VERSION 11500

// Currently, the impl of hashmap relies on the cuCollective library, which
// requires CUDA 12.0+
...
nv_gpu_shared_hash_map_impl::nv_gpu_shared_hash_map_impl(
	boost::interprocess::managed_shared_memory &memory, uint64_t max_entries,
	uint64_t key_size, uint64_t value_size)
	: agent_gpu_shared_mem(memory.get_segment_manager()), key_size(key_size),
	  ...
	  value_buffer(memory.get_segment_manager())
{
	...
	if (cuda_version < MINIMUM_CUDA_VERSION) {
		SPDLOG_ERROR(
			"CUDA driver version {} is too old; need at least 12.0",
			cuda_version);
		throw std::runtime_error(
			"CUDA driver version too old; need at least 12.0");
	}
	...
	SPDLOG_INFO(
		"Initializing map type of BPF_MAP_TYPE_GPU_HASH_MAP (device), total_buffer_size={}",
		total_buffer_size);
	if (auto err = cuMemAlloc(&server_gpu_shared_mem, total_buffer_size);
	    err != CUDA_SUCCESS) {
		SPDLOG_ERROR(
			"Unable to allocate GPU buffer for nv_gpu_shared_hash_map_impl: {}",
			(int)err);
		throw std::runtime_error(
			"Unable to allocate GPU buffer for nv_gpu_shared_hash_map_impl");
	}
	...
	if (auto err = cuIpcGetMemHandle(&this->gpu_mem_handle,
	    ...
	    err != CUDA_SUCCESS) {
		SPDLOG_ERROR(
			"Unable to open CUDA IPC handle for nv_gpu_shared_hash_map_impl: {}",
			(int)err);
		throw std::runtime_error(
			"Unable to open CUDA IPC handle for nv_gpu_shared_hash_map_impl");
	}
...
nv_gpu_shared_hash_map_impl::~nv_gpu_shared_hash_map_impl()
{
	if (shm_holder.global_shared_memory.get_open_type() !=
		...
		if (auto err = cuMemFree(server_gpu_shared_mem);
		    err != CUDA_SUCCESS) {
			SPDLOG_WARN(
				"Unable to free CUDA memory for nv_gpu_shared_hash_map_impl: {}",
				(int)err);
		}
		if (did_switch_ctx) {
			cuCtxSetCurrent(prev_ctx);
...
nv_gpu_shared_hash_map_impl::try_initialize_for_agent_and_get_mapped_address()
{
	if (shm_holder.global_shared_memory.get_open_type() ==
		...
		if (auto itr = agent_gpu_shared_mem.find(pid);
			...
			SPDLOG_INFO(
				"Initializing nv_gpu_shared_hash_map_impl at pid {}",
				pid);
			CUdeviceptr ptr;
