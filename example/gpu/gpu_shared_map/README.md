...
This example demonstrates the behavior of `BPF_MAP_TYPE_GPU_ARRAY_MAP` (a normal, non-per-thread, single-copy shared array) and `BPF_MAP_TYPE_GPU_HASH_MAP` (a normal, non-per-thread, single-copy shared array) in user space in conjunction with bpftime. This example relies on the agent side for updates, while the server only reads and does not write back to the HOST.
...
```bash
/root/bpftime_sy03/bpftime/build/tools/cli/bpftime load /root/bpftime_sy03/bpftime/example/gpu/gpu_shared_map/gpu_shared_map
```
